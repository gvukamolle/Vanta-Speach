...
enum class RecordingPreset(
    ...
    val systemPrompt: String
        get() = when (this) {
            SALES_CALL -> SALES_CALL_PROMPT
            PROJECT_MEETING -> PROJECT_MEETING_PROMPT
            DAILY_STANDUP -> DAILY_STANDUP_PROMPT
            INTERVIEW -> INTERVIEW_PROMPT
            FAST_IDEA -> FAST_IDEA_PROMPT
        }
    ...
    companion object {
        ...
        private const val SALES_CALL_PROMPT = """Ты — ассистент для обработки записей звонков с клиентами (sales calls). Твоя задача — извлечь ключевую информацию для отдела продаж.
...
❌ "Задача: отправить КП" — если менеджер не обещал этого явно"""
        ...
        private const val PROJECT_MEETING_PROMPT = """Ты — ассистент для обработки записей проектных встреч. Твоя задача — превратить транскрипцию в рабочий документ для команды.
...
❌ Секция "Блокеры" с текстом "Блокеров не выявлено" — просто не добавляй секцию"""
        ...
        private const val DAILY_STANDUP_PROMPT = """Ты — ассистент для обработки записей ежедневных стендапов и синков команды.
...
- Если стендап превратился в обсуждение проблемы — это нормально, опиши суть обсуждения."""
        ...
        private const val INTERVIEW_PROMPT = """Ты — ассистент для обработки записей собеседований. Твоя задача — структурировать информацию для принятия решения о найме.
...
❌ "Красные флаги: недостаточно опыта" — это мнение, не флаг. Флаг: "сказал 5 лет опыта, но не смог объяснить базовые концепции""""
        ...
        private const val FAST_IDEA_PROMPT = """Ты — ассистент для обработки голосовых заметок. Твоя задача — превратить надиктованное в чистый, удобный текст.
